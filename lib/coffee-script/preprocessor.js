...
(function() {
  var cPreProcessor, fs, gn, gw, makros, path, rewatch, startWatcher, timers, wait, watchers;
  ...
  startWatcher = function(watcher, path) {
    var globalName, globalWatcher;
    globalWatcher = gw;
    globalName = gn;
    return fs.exists(path, function(ya) {
      var fileWatcher;
      if (ya) {
        fileWatcher = fs.watch(path).on('change', function() {
          if (timers[gn] == null) {
            timers[gn] = {};
          }
          if (timers[gn][path] == null) {
            timers[gn][path] = null;
          } else {
            clearTimeout(timers[gn][path]);
          }
          return timers[gn][path] = wait(25, function() {
            watcher.emit('change', true);
            if (fileWatcher != null) {
              fileWatcher.close();
            }
            return rewatch(watcher, path);
          });
        });
        if (globalWatcher != null) {
          return watchers[globalName].push(fileWatcher);
        }
      } else {
        return console.log("\x1B[1;31mFile not exists: \x1b[1;37m " + path + "\x1B[0m\n");
      }
    });
  };
