...
(function() {
  ...
  cPreProcessor = function(source, filename, indent) {
    var ad, an, ba, buf, c, c2, cl, d, def, defFull, define, di, directives, dm, ecran, error, f, i, i2, ifState, ifactive, include, isw, l, line, n, nfound, os, out, q, replace, s, sd, sl, space, token, undef, warn, word, wr;
    if (indent == null) {
      indent = "";
    }
    ...
    while (i++ < l) {
      ...
      ba = ifactive === 0 || ifState[ifactive][0];
      if (!ba) {
        s = true;
      }
      if (!ecran) {
      ...
      if (d) {
        ...
        if (c !== '\n') {
          ...
          if (ifactive > 0 && !ifState[ifactive][0] && ad < 3) {
            i = source.indexOf('\n', i) - 1;
            d = false;
            ad = -1;
            line++;
          ...
          switch (ad) {
            ...
            case 4:
              ...
              if (ifState[ifactive][1]) {
                ifState[ifactive][0] = false;
              } else if (!ifState[ifactive - 1][0]) {
                ifState[ifactive][0] = ifState[ifactive][1] = false;
              } else if (eval(word)) {
                ifState[ifactive][1] = ifState[ifactive][0] = true;
              }
              break;
            case 5:
              if (ifactive === 0) {
              ...
              if (!ifState[ifactive - 1][0]) {
                ifState[ifactive][0] = ifState[ifactive][1] = false;
              } else if (!ifState[ifactive][1]) {
                ifState[ifactive][0] = ifState[ifactive][1] = true;
              }
              break;
            case 6:
              if (ifactive === 0) {
            ...
            case 7:
              ...
              ifState[ifactive] = [false, false];
              if (ifState[ifactive - 1][0]) {
                ifState[ifactive][0] = ifState[ifactive][1] = makros.hasOwnProperty(word) && (makros[word] != null);
              }
              break;
            case 8:
              ifactive++;
  ...
  optionParse = function(opts) {
    var dirs, externalDefine, o, sp, _j, _k, _len1, _len2, _ref;
    _ref = opts.params;
